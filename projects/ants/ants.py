...
class Ant(Insect):
    ...
    def buff(self):
        ...
        if self.name != 'Fire' and self.damage == 1:
            self.damage *= 2
        elif self.name == 'Fire' and self.damage == 3:
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        transition_count = 0
        while place != beehive:
            if place.bees and self.min_range <= transition_count <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health > 0:
            for bees in bees_copy:
                Ant.reduce_health(bees, amount)
        else:
            for bees in bees_copy:
                Ant.reduce_health(bees, amount + self.damage)
        # END Problem 5

# BEGIN Problem 6
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn, but it is useful to absorb damage with its high health value."""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt chews a random bee in its place that dies and takes three turns to digest it.
    If there is no bee in its place, the HungryAnt does nothing, and it cannot eat another bee while digesting.
    """
    name = 'Hungry'
    food_cost = 4
    chew_duration = 3
...
class TankAnt(ContainerAnt):
    """TankAnt is a ContainerAnt that can also deal 1 damage to all bees in its place each turn."""
    ...
    def action(self, gamestate):
        ...
        if self.place.bees:
            for bees in bees_copy:
                Ant.reduce_health(bees, self.damage)
...
# END Problem EC
    ...
    def __init__(self, health=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(health)
        self.label = QueenAnt.Queen_Count
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.health == 0 or (self.place.exit == None and self.place.bees):
            bees_win()

        # END Problem EC
    def remove_from(self, place):
        if place.ant is self and self.label != 0:
            place.ant = None
        elif place.ant is None:
            assert False, '{0} is not in {1}'.format(self, place)
        else:
