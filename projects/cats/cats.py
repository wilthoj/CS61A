...
def accuracy(typed, reference):
    ...
    if typed_words == '' and reference_words == '':
        ...
        for item in typed_words:
            for elem in reference_words:
                if item == elem:
                    equals += 1
                index +=1
        return float((equals/len(reference_words))*100)'''
    "*** YOUR CODE HERE ***"
    i, j, match, total = 0, 0, 0, len(typed_words)
    if typed == reference:
        return 100.0
    elif len(typed_words) == 0:
        return 0.0
    while len(typed_words) > len(reference_words):
        typed_words.pop()
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        diff = min(valid_words, key=lambda word: diff_function(typed_word, word, limit))
        if limit<diff_function(typed_word,diff,limit):
            return typed_word
        else:
            return diff
...
def report_progress(typed, prompt, user_id, send):
    ...
    for index in range(len(typed)):
        if typed[index]==prompt[index]:
            ...
            break
    progress = float(correct/len(prompt))
    report = {
        'id': user_id,
        'progress': progress,
    }
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    elasped_total = []
    for elem in range(len(times_per_player)):
        for index in range(1,(len(times_per_player[elem]))):
            elasped_total.append((times_per_player[elem][index]-times_per_player[elem][index-1]))
    #splitting up the list into equal # of subparts dependent on the lengtho f times_per_player

    if len(times_per_player)==1:
        split_elasped = [elasped_total]
    elif len(times_per_player)==2:
        split_elasped = [elasped_total[:len(elasped_total)//2], elasped_total[len(elasped_total)//2:]]
    else:
        split_elasped = [elasped_total[:len(elasped_total)//3], elasped_total[len(elasped_total)//3:(len(elasped_total)//3)*2], elasped_total[2*len(elasped_total)//3:]]
    return game(words, split_elasped)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for w in word_indices:
        fastestPlayer = 0
        fastestTime = time(game, fastestPlayer, w)
        for p in player_indices:
            if time(game, p, w) < fastestTime:
                fastestPlayer = p
                fastestTime = time(game, p, w)
        result[fastestPlayer].append(word_at(game, w))
    return result
    # END PROBLEM 10
