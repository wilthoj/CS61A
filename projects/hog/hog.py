...
def piggy_points(score):
    ...
    if sqr<10:
        return a + 3
    elif sqr>10 and sqr<=100:
        k=min(a,b)
        return k + 3
    elif sqr>100 and sqr<=1000:
        k=min(a,b,c)
        return k + 3
    elif sqr>1000 and sqr<=10000:
        k=min(a,b,c,d)
        return k + 3
    else:
        k=min(a,b,c,d,e)
...
def more_boar(player_score, opponent_score):
    ...
    return left_most_digit(player_score) < left_most_digit(opponent_score) and second_left_digit(player_score) < second_left_digit(opponent_score)
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 0:
            if score0 - ls > rh:
                rh = score0 - ls
                print('Player', who, 'has reached a new maximum point gain.', rh, 'point(s)!')
            ls = score0
        else:
            if score1 - ls > rh:
                rh = score1 - ls
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_roll = 0 # arbitrary
    i = 1 # index for the while loop
    num_dice = 0 # also arbitrary; the variable to be returned
    average_dice = make_averaged(roll_dice, trials_count)
    while i <= 10:
        current_average = average_dice(i, dice)
        if current_average > max_roll:
            max_roll = current_average
            num_dice = i
        i += 1
    return num_dice
    # END PROBLEM 9
