...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda arg: scheme_eval(arg, env))                  #[operand.eval(env) for operand in self.operands]
...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        new_binding = {symbol: value}
        self.bindings.update(new_binding)
        # END PROBLEM 2
