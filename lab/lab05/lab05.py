...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    ref_dist1=sqrt((get_lat(city_a)-lat)**2+(get_lon(city_a)-lon)**2)
    ref_dist2=sqrt((get_lat(city_b)-lat)**2+(get_lon(city_b)-lon)**2)
    if ref_dist1<ref_dist2:
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for leaves in branches(t):
            if berry_finder(leaves):
                return True
        return False
